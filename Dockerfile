...
RUN wget --no-verbose -O ${MAVEN_TEMP}/apache-maven.tar.gz https://archive.apache.org/dist/maven/maven-3/${MAVEN_VERSION}/binaries/apache-maven-${MAVEN_VERSION}-bin.tar.gz && tar xzf /tmp/apache-maven.tar.gz -C /opt/ && ln -s /opt/apache-maven-${MAVEN_VERSION} ${MAVEN_HOME} && rm -f ${MAVEN_TEMP}/apache-maven.tar.gz
